...
```{r dataframe, echo=FALSE, comment = '', warning = FALSE, message = FALSE}
library(mcr)
library(pander)
a <- datasetInput()
if (is.null(a)) {
...
pander(MCResult.getData(data1), caption = tname)
}
```
...
method.bootstrap.ci = input$metbootci,
mref.name = input$var1, mtest.name = input$var2)
pander(getCoefficients(data1), caption = "Estimates (EST), standard error (SE) and upper-lower confidence intervals (UCI-LCI) for intercept and slope.")
}
```
